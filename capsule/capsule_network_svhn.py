...
def softmax(input, dim=1):
    ...
    softmaxed_output = F.softmax(transposed_input.contiguous().reshape(-1, transposed_input.size(-1)), dim=-1)
    return softmaxed_output.reshape(*transposed_input.size()).transpose(dim, len(input.size()) - 1)
...
if __name__ == "__main__":
    ...
    confusion_logger = VisdomLogger('heatmap',annot=True, opts={'title': 'Confusion matrix',
