...
weight = torch.randn(1, device='cuda')
class CapsuleNet(nn.Module):
    def __init__(self):
        super(CapsuleNet, self).__init__()

        self.conv1 = nn.Conv2d(in_channels=1, out_channels=256, kernel_size=9, stride=1)
        # self.m = nn.BatchNorm2d(256)
        self.m = nn.Dropout(p=0.2)
        self.conv2 = nn.Conv2d(in_channels=256, out_channels=256, kernel_size=9, stride=1)
        self.primary_capsules = CapsuleLayer(num_capsules=8, num_route_nodes=-1, in_channels=256, out_channels=32,
                                             kernel_size=9, stride=2)
        self.digit_capsules = CapsuleLayer(num_capsules=NUM_CLASSES, num_route_nodes=28800, in_channels=8,
                                           out_channels=16)
    ...
    def forward(self, x, y=None):
        ...
        x = F.prelu(self.conv1(x),weight)
        # x=self.m(x)
        x = F.prelu(self.conv2(x),weight)
        # x=self.m(x)
        x = F.prelu(self.conv2(x),weight)
        # x=self.m(x)
        x = F.prelu(self.conv2(x),weight)        
...
class CapsuleLoss(nn.Module):
    ...
    def forward(self, images, labels, classes, reconstructions):
        left = F.prelu(0.9 - classes, weight) ** 2
        right = F.prelu(classes - 0.1, weight) ** 2
...
if __name__ == "__main__":
    ...
    def get_iterator(mode):
        ...
        #             labels.append(idx)
        #     return imgs,labels
        # org,label=read_img(path)
        # org=np.expand_dims(org,3) # add channel dimension, 4D
        ...
        if mode ==True: 
            data=data_train.reshape(3955,100, 100)
            data=torch.from_numpy(data)       
            labels=torch.from_numpy(labels_train)
            labels=labels.type(torch.LongTensor)
        else: 
            data=data_test.reshape(396,100, 100)
            data=torch.from_numpy(data)       
    ...
    def on_end_epoch(state):
        ...
        confusion_logger.log(confusion_meter.value().astype(int))
    ...
    dict = {'epochs': epochs, 'train_loss': train_loss, 'test_loss': test_loss,'train_accuracy':train_accuracy,'test_accuracy':test_accuracy}  
       
    df = pd.DataFrame(dict) 
    
    # saving the dataframe 
    df.to_csv('finalaug.csv') 
    a = np.array([epochs,train_loss,train_accuracy,test_loss,test_accuracy])
    np.savetxt('myfileaug.csv', a, delimiter=',')
